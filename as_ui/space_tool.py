...
def draw_alva_toolbar(self, context):
    ...
    if space_type == 'SEQUENCE_EDITOR' and scene.view_sequencer_toolbar:
        flow.operator("alva_sequencer.add", icon='FILE_TEXT', text="Macro" if region_width > 200 else "").Option = "option_macro"
        flow.operator("alva_sequencer.add", icon='PLAY', text="Cue" if region_width > 200 else "").Option = "option_cue"
